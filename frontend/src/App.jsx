...
const IconLogo = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-cyan-400 filter drop-shadow-[0_0_8px_rgba(34,211,238,0.5)]"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
)
const IconBack = () => (
  ...
  <>
    ...
    <div className="bg-slate-950/80 backdrop-blur-md border-b border-white/5 text-center py-1.5 z-[60] relative">
        <div className="container mx-auto px-4 flex justify-between items-center text-[10px] uppercase tracking-widest font-bold text-cyan-500/80">
            <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-[pulse_2s_infinite]"></span> System Online</span>
            <span className="hidden sm:inline opacity-75">Pi-Guardian v2.0.0 • Protected</span>
    ...
    <nav className="sticky top-0 z-50 mb-8 border-b border-white/5 bg-slate-900/60 backdrop-blur-2xl supports-[backdrop-filter]:bg-slate-900/60">
      <div className="container mx-auto px-4">
        <div className="flex h-20 items-center justify-between">
          <a className="flex items-center gap-3 text-2xl font-bold text-white tracking-tight" href="#">
            <IconLogo />
            <span className="bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Pi-Guardian</span>
          </a>
...
const GlassCard = ({ children, className = "" }) => (
    <div className={`relative overflow-hidden bg-slate-900/40 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl shadow-black/20 ${className}`}>
...
const OverviewCards = ({ deviceCount, dnsCount, trafficIn, trafficOut }) => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <GlassCard>
      <div className="text-cyan-400 text-xs font-bold uppercase tracking-wider mb-2">Devices seen</div>
      <div className="flex items-baseline justify-between">
        <div className="text-4xl font-light text-white tracking-tight">{deviceCount}</div>
    ...
    <GlassCard>
      <div className="text-purple-400 text-xs font-bold uppercase tracking-wider mb-2">DNS domains</div>
      <div className="flex items-baseline justify-between">
        <div className="text-4xl font-light text-white tracking-tight">{dnsCount}</div>
    ...
    <GlassCard>
      <div className="text-emerald-400 text-xs font-bold uppercase tracking-wider mb-2">Current traffic</div>
      <div className="flex justify-between items-end">
        <div>
          <div className="text-slate-500 text-[10px] uppercase mb-1">Inbound</div>
          <div className="text-xl font-semibold text-white">{trafficIn} <span className="text-sm text-slate-500">KB/s</span></div>
        </div>
        <div className="text-right">
          <div className="text-slate-500 text-[10px] uppercase mb-1">Outbound</div>
          <div className="text-xl font-semibold text-white">{trafficOut} <span className="text-sm text-slate-500">KB/s</span></div>
        </div>
      </div>
...
const TrafficMonitor = ({ devices, selectedDevice, onDeviceSelect, onBack, trafficData, dnsData, protocolData, timeFilter, setTimeFilter }) => {
    // Chart Config
    const chartOptions = {
        responsive: true, maintainAspectRatio: false,
        scales: { 
            x: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#64748b', font: { family: 'Inter', size: 10 } } }, 
            y: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#64748b', font: { family: 'Inter', size: 10 } } } 
        },
        plugins: { legend: { labels: { color: '#94a3b8', font: { family: 'Inter', size: 11 }, usePointStyle: true, boxWidth: 6 } } },
    ...
    const trafficChartData = {
        ...
        datasets: [
            { label: 'Download', data: trafficData.map(d => d[1] / 1024), borderColor: '#22d3ee', backgroundColor: (context) => {
            ...
            { label: 'Upload', data: trafficData.map(d => d[2] / 1024), borderColor: '#f472b6', backgroundColor: (context) => {
                const ctx = context.chart.ctx;
                const gradient = ctx.createLinearGradient(0, 0, 0, 300);
    ...
    const dnsChartData = {
        ...
        datasets: [{ 
            ...
            backgroundColor: ['#22d3ee', '#818cf8', '#c084fc', '#f472b6', '#fb7185'], 
            borderWidth: 0,
            hoverOffset: 10
        }]
    };
        ...
        <div className="space-y-6">
                ...
                <div>
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        <h2 className="text-3xl font-bold text-white tracking-tight">
                            {selectedDevice ? selectedDevice.hostname : 'Global Network Traffic'}
                        </h2>
                    </div>
                    {selectedDevice ? (
                        ...
                        <p className="text-slate-400 text-sm ml-10">Real-time network traffic and DNS analytics for the entire network.</p>
                    )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <GlassCard className="!p-5">
                    <div className="text-emerald-400 text-[10px] uppercase font-bold tracking-widest mb-1">Status</div>
                ...
                <GlassCard className="!p-5">
                    <div className="text-purple-400 text-[10px] uppercase font-bold tracking-widest mb-1">Total Domains</div>
                    <div className="text-lg font-bold text-white">{dnsData.length}</div>
                </GlassCard>
                <GlassCard className="!p-5">
                    <div className="text-cyan-400 text-[10px] uppercase font-bold tracking-widest mb-1">Peak Traffic</div>
                    <div className="text-lg font-bold text-white">{peakTraffic} <span className="text-sm text-slate-500 font-medium">KB/s</span></div>
                </GlassCard>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <GlassCard className="lg:col-span-2">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex gap-2">
                            <span className="flex items-center gap-1.5 text-[10px] text-slate-400 uppercase font-bold"><span className="w-2 h-2 rounded-full bg-cyan-400"></span> In</span>
                            <span className="flex items-center gap-1.5 text-[10px] text-slate-400 uppercase font-bold"><span className="w-2 h-2 rounded-full bg-pink-400"></span> Out</span>
                        </div>
                    </div>
                    <div className="h-[300px]"><Line data={trafficChartData} options={{...chartOptions, plugins: { legend: { display: false } }}} /></div>
                </GlassCard>
                <GlassCard>
                    ...
                    <div className="h-[200px] flex items-center justify-center relative">
                        <Doughnut data={dnsChartData} options={{ maintainAspectRatio: false, cutout: '70%', plugins: { legend: { display: false } } }} />
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className="text-center">
                                <div className="text-2xl font-bold text-white">{dnsData.length}</div>
                                <div className="text-[10px] text-slate-500 uppercase tracking-wider">Domains</div>
                            </div>
                        </div>
                    </div>
                        ...
                        {dnsData.slice(0,3).map((d, i) => (
                            <div key={i} className="flex items-center justify-between text-xs">
                                <div className="flex items-center gap-2">
                                    <span className={`w-2 h-2 rounded-full`} style={{backgroundColor: ['#22d3ee', '#818cf8', '#c084fc'][i]}}></span>
                ...
                <GlassCard className="lg:col-span-3">
                    ...
                    <div className="h-[200px]"><Bar data={protocolChartData} options={{...chartOptions, plugins: { legend: { display: false } }}} /></div>
                </GlassCard>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-slate-300">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {dnsData.map((d, i) => (
                                <tr key={i} className="hover:bg-white/5 transition-colors">
                                    <td className="px-6 py-3 font-medium text-white">{d.domain}</td>
                                    <td className="px-6 py-3 text-slate-400">{d.count}</td>
                                    <td className="px-6 py-3 text-slate-500 font-mono text-xs">{new Date(d.last_seen * 1000).toLocaleTimeString()}</td>
                                </tr>
                            ))}
                            {dnsData.length === 0 && <tr><td colSpan="3" className="px-6 py-8 text-center text-slate-500 italic">No data available</td></tr>}
                        </tbody>
                    </table>
                </div>
            </GlassCard>
...
const Settings = () => (
    <GlassCard className="max-w-2xl mx-auto">
        ...
        <p className="text-slate-400 text-sm mb-8">Configuration options for your Pi-Guardian instance.</p>
        <div className="space-y-6">
            <div>
                <label className="block text-sm font-bold text-slate-300 mb-2">Auto-Scan Interval (seconds)</label>
                <input type="number" className="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all" defaultValue="60" />
            </div>
            <button className="px-6 py-3 bg-cyan-500 hover:bg-cyan-400 text-slate-950 rounded-xl transition-all font-bold shadow-[0_0_20px_rgba(6,182,212,0.3)] w-full sm:w-auto">Save Changes</button>
        </div>
    </GlassCard>
)
